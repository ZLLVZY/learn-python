...
row_data={}
with open('PaceData.xlsx') as paces:
    ...
    for each_line in paces:
        ...
        row_label=row.pop(0)
        inner_dict={}
        for n in range(len(column_headings)):
            inner_dict[format_time(row[n])]=column_headings[n]
...
prediction=[k for k in row_data[predicted_distance].keys() if row_data[predicted_distance][k]==closet_column_heading]
'''print(column_heading)
prediction=[k for k in row_data['20k'].keys() if row_data['20k'][k]==column_heading]
print(find_nearest_time('1:01:01',['1:00:00','2:00:00','1:01:00']))'''
print(prediction)